...
from app.models.schemas.organisation import (
...
async def create_organisation(
    name: str, user_id: uuid.UUID, db: AsyncSession = Depends(get_db)
) -> OrganisationSchema:
...
async def get_services_by_organization_id(
    organization_id: uuid.UUID, db: AsyncSession = Depends(get_db)
) -> List[ServiceSchema]:
    ...
    return [
        ServiceSchema(
            id=s.id, name=s.name, organization_id=s.organization_id, status=s.status
        )
        for s in services_list
    ]
...
async def update_organization(
    organization_id: uuid.UUID, name: str, db: AsyncSession = Depends(get_db)
) -> OrganisationSchema:
    org_query = select(Organization).where(Organization.id == organization_id)
    org_resp = await db.execute(org_query)
    org_found = org_resp.scalar_one_or_none()
    ...
    return OrganisationSchema(
        id=org_found.id, name=org_found.name, user_id=org_found.user_id
    )
